...
class Pagination
{
     ...
     * One config group can
     * refer to another as its parent, which will be recursively loaded.
     *
     * @param
     *            string pagination config group; "default" if none given
    ...
    public function setup(array $config = array())
    {
        ...
        if ($this->current_page === NULL or isset ($config ['current_page']) or isset ($config ['total_items']) or isset ($config ['items_per_page'])) {
            ...
            //TODO::mclkim $this->current_first_item = (int)min((($this->current_page - 1) * $this->items_per_page) + 1, $this->total_items);
            $this->current_first_item = (int)min((($this->current_page - 1) * $this->items_per_page), $this->total_items);
    ...
    public function url($page = 1)
    {
        ...
        switch ($this->config ['current_page'] ['source']) {
            case 'query_string' :
                return URL::site(Request::current()->uri()) . URL::query(array(
                        $this->config ['current_page'] ['key'] => $page
                    ));

            case 'route' :
                return URL::site(Request::current()->uri(array(
                        $this->config ['current_page'] ['key'] => $page
                    ))) . URL::query();

            case 'mixed' :
                return URL::site(Request::detect_uri()) . URL::query(array(
                        $this->config ['current_page'] ['key'] => $page
                    ));
        }
     ...
     *
     * @param
     *            mixed string of the view to use, or a Kohana_View object
